...
//		PrintWriter gravarArqSaiu = new PrintWriter(arqSaiu);
		...
		int cont = 1;
		URL resultado = Resultado2025.class.getClassLoader().getResource("hoje.csv");
...
//            	String lista = null;
            	int qtd = fechamento2025(linhaResultado);
...
//            gravarArqSaiu.close();
	...
	@SuppressWarnings({ "deprecation" })
	public static int fechamento2025(int[] linhaResultado) throws URISyntaxException, NumberFormatException, IOException {
		
		int retorno = 0;
		int contJogo = 0;
		int contLinha = 0;
		URL combinacoes2025 = Resultado2025.class.getClassLoader().getResource("combinacoes\\20_25.csv");
		if (Objects.nonNull(combinacoes2025)) {
			
			Path caminho = Paths.get(combinacoes2025.toURI());
            CSVReader csvReader = new CSVReader(new FileReader(caminho.toFile()), ',');
            String[] linhaCombinacoes2025;
            while (Objects.nonNull((linhaCombinacoes2025 = csvReader.readNext()))) {
            	
            	contJogo = 0;
            	contLinha++;
            	int[] linha2025 = new int[linhaCombinacoes2025.length];
            	for (int i = 0; i < linhaCombinacoes2025.length; i++) {
            		linha2025[i] = Integer.parseInt(String.valueOf(linhaCombinacoes2025[i]));
        		}
            	
            	for (int i = 0; i < linhaResultado.length; i++) {
            		for (int j = 0; j < linha2025.length; j++) {
            			if (linhaResultado[i] == linha2025[j]) {
            				contJogo++;
            			}
            		}
            	}
